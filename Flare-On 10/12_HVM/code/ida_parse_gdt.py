...
#
# IDA script to parse and annotate the GDT tables
# at 0xd0e size 0x16
#
# based on https://github.com/inforion/idapython-cheatsheet/blob/master/misc/parse_gdt.py
#
...
def parse_access_byte(acc):
    ...
    rw = (acc & 0x2) >> 1
    ac = acc & 0x1
    res = "      Access: Pr {:b} Privl {:02x} Ex {:b} DC {:b} RW {:b} Ac {:b}".format(
        pr, privl, ex, dc, rw, ac
    )
    return res
...
def parse_GDT_record(bytes):
    ...
    base_str = "Base {:08x} Limit {:08x} Access {:02x} Flags {:02x}".format(
        base, limit, acc, flags
    )
...
# Parse gdt_pm32 @ 0xd0e
parse_GDT_table(0xD0E, 0xD25)
# Parse gdt_lm64 @ 0xd2c
parse_GDT_table(0xD2C, 0xD43)
