...
def log_block(log_file_base, current_part, block_number, inst_log, next_block_file):
    ...
    file_name = f"{log_file_base}_part_{current_part}_block_{block_number:04}_rva_0x{inst_log['handler_rva']:06x}_unwind_{inst_log['runtime_function_unwindinfo']:06x}"
    ...
    with open(next_block_file, "w") as f:
        next_block_rva = inst_log['handler_rva']
        ...
        save_line = f"{block_number}|{next_block_rva}|{inst_log['saved_rsp']}|{runtime_function_start}|{runtime_function_end}|{runtime_function_unwindinfo}"
        f.write(save_line)
    f.close()
...
def dump_blocks_new(shellcode_base, exec_handler, RtlLookupFunctionEntryReturn, log_base, next_block_file, patch_target, patch_file_offset):
    ...
    next_block, next_block_rva, saved_rsp, rf_start, rf_end, rf_unwind = parse_next_block_file(next_block_file)
    if rf_unwind != 0:
    ...
    handler_va, handler_rva, current_part = skip_blocks(shellcode_base, exec_handler, next_block, next_block_rva)
    print(f"[+] part {current_part} handler VA {handler_va:x} RVA {handler_rva:x}")
    ...
    while (current_part < 6):
    # for i in range(3):
        next_part, inst_log = process_block(current_part, shellcode_base, exec_handler, current_block, handler_va, handler_rva)
        ...
        inst_log = deobfuscate_block(current_block, inst_log)
        
        # log the block and update the next_block file
        log_block(log_base, current_part, current_block, inst_log, next_block_file)

        # patch the target shellcode area with the deobfuscated code
        patch(patch_target, patch_file_offset, inst_log, current_block)

        # do some kind of end check - have we reached the end?
        if (next_part == 6):
            print(f"[*] We reached jmpni r15, congrats!")
            # double check that we are where we expect to be
            ida_dbg.wait_for_next_event(ida_dbg.WFNE_SUSP, -1)
            where_am_i = idautils.cpu.Rip
            assert where_am_i == shellcode_base + 0x72319
            final_jmp_target = idautils.cpu.R15
            print(f"[*] Final jump target r15: {final_jmp_target:x} [0x1400011F0 = lose, 0x1400011B0 = win]")
            exit(-1)
        ...
        idaapi.step_into()
        ida_dbg.wait_for_next_event(ida_dbg.WFNE_SUSP, -1)
        # we assert that we are at the right location at the start of the next process_block() call
        current_block += 1
...
def parse_next_block_file(filename):
    ...
    try:
        with open(filename, "r") as f:
            ...
            next_block_rva = int(saved_line[1])
            saved_rsp = int(saved_line[2])
            runtime_function_start = int(saved_line[3])
            runtime_function_end = int(saved_line[4])
            runtime_function_unwindinfo = int(saved_line[5])
        f.close()
    except Exception as e:
    ...
    return (next_block, next_block_rva, saved_rsp, runtime_function_start, runtime_function_end, runtime_function_unwindinfo)


def skip_blocks(shellcode_base, exec_handler, next_block, next_block_rva):
    # skip previously dumped blocks - likely because ida crashed again
    current_part = 1
    if next_block == 0:
        # run to first exec_handler
        handler_va, handler_rva = run_to_next_RtlpExecuteHandlerForException(exec_handler, shellcode_base)
        # we need to run into the shellcode first block 0
        print(f"[+] Starting fresh at block {next_block} at VA {handler_va:x} RVA {handler_rva:x}")
        ida_dbg.run_to(handler_va)
        ida_dbg.wait_for_next_event(ida_dbg.WFNE_SUSP, -1)
        return (handler_va, handler_rva, current_part)
    # if next_block > 363
    # take short_cut to skip part 1
    # special treatment for last block in part 1, we need to cheat around
    # the test r14, r14 instruction at rva 0x0179FD
    if next_block > 363:
        test_r14_va = shellcode_base + 0x0179FD
        print(f"[+] Skipping part 1 by cheating test r14 check in block 363 at {test_r14_va:x}")
        ida_dbg.run_to(test_r14_va)
        ida_dbg.wait_for_next_event(ida_dbg.WFNE_SUSP, -1)
        idc.set_reg_value(0, "R14")
        current_part = 2
    # skip part 2
    if next_block > 724:
        test_rbx_va = shellcode_base + 0x2F386
        print(f"[+] Skipping part 2 by cheating test rbx check in block 724 at {test_rbx_va:x}")
        ida_dbg.run_to(test_rbx_va)
        ida_dbg.wait_for_next_event(ida_dbg.WFNE_SUSP, -1)
        idc.set_reg_value(0, "RBX")
        current_part = 3
    # skip part 3
    if next_block > 1097:
        test_r12_va = shellcode_base + 0x4751A
        print(f"[+] Skipping part 3 by cheating test r12 check in block 1097 at {test_rbx_va:x}")
        ida_dbg.run_to(test_r12_va)
        ida_dbg.wait_for_next_event(ida_dbg.WFNE_SUSP, -1)
        idc.set_reg_value(0, "R12")
        current_part = 4
    # skip part 4
    if next_block > 1434:
        test_rsi_va = shellcode_base + 0x5D2CD
        print(f"[+] Skipping part 4 by cheating test rsi check in block 1434 at {test_rsi_va:x}")
        ida_dbg.run_to(test_rsi_va)
        ida_dbg.wait_for_next_event(ida_dbg.WFNE_SUSP, -1)
        idc.set_reg_value(0, "Rsi")
        current_part = 5

    # now run to the next block handler
    handler_rva = next_block_rva
    handler_va = shellcode_base + handler_rva
    print(f"[+] Skipping to block {next_block} at VA {handler_va:x} RVA {handler_rva:x}")
    ida_dbg.run_to(handler_va)
    ida_dbg.wait_for_next_event(ida_dbg.WFNE_SUSP, -1)
    return (handler_va, handler_rva, current_part)


def process_block(current_part, shellcode_base, exec_handler, next_block, handler_va, handler_rva):
    # do stuff
    block_counter = next_block
    print(f"-----\nBegin dumping part {current_part} block {block_counter} @ VA:{handler_va:x} RVA:{handler_rva:x}\n----")
    # log_trace = False # may be used for logic to toggle skipping junk code and to only log legit code
    # skip_one = False

    # we should be at the handler_va now
    # double check that before ida goes boom
    ida_dbg.wait_for_next_event(ida_dbg.WFNE_SUSP, -1)
    where_am_i = idautils.cpu.Rip
    assert where_am_i == handler_va
    ...
    # jmp near rbp end of part 2
    while (inst.itype != idaapi.NN_hlt) and (current_part < 6):
        ...
        # in block 363 at rva 0x0179FD, we encounter a test r14, r14
        # if r14 != 0, the code bails out to wrong key
        # let's intercept this and set r14 = 0 to pass this test in order to dump further
        if (inst.ea - shellcode_base == 0x0179FD and inst.itype == idaapi.NN_test and inst[0].type == idaapi.o_reg and inst[0] == idautils.procregs.R14 and inst[1].type == idaapi.o_reg and inst[1] == idautils.procregs.R14):
            idc.set_reg_value(0, "R14")

        # in block 724 at rva 0x2F386, we encounter a test rbx, rbx
        # if rbx != 0, the code bails out to wrong key
        # let's intercept this and set rbx = 0 to pass this test in order to dump further
        if (inst.ea - shellcode_base == 0x2F386 and inst.itype == idaapi.NN_test and inst[0].type == idaapi.o_reg and inst[0] == idautils.procregs.Rbx and inst[1].type == idaapi.o_reg and inst[1] == idautils.procregs.Rbx):
            idc.set_reg_value(0, "Rbx")

        # in block 1097 at rva 0x4751A, we encounter a test r12, r12
        # if r12 != 0, the code bails out to wrong key
        # let's intercept this and set r12 = 0 to pass this test in order to dump further
        if (inst.ea - shellcode_base == 0x4751A and inst.itype == idaapi.NN_test and inst[0].type == idaapi.o_reg and inst[0] == idautils.procregs.R12 and inst[1].type == idaapi.o_reg and inst[1] == idautils.procregs.R12):
            idc.set_reg_value(0, "R12")

        # in block 1434 at rva 0x5D2CD, we encounter a test rsi, rsi
        # if rsi != 0, the code bails out to wrong key
        # let's intercept this and set rsi = 0 to pass this test in order to dump further
        if (inst.ea - shellcode_base == 0x5D2CD and inst.itype == idaapi.NN_test and inst[0].type == idaapi.o_reg and inst[0] == idautils.procregs.Rsi and inst[1].type == idaapi.o_reg and inst[1] == idautils.procregs.Rsi):
            idc.set_reg_value(0, "Rsi")
        ...
        inst_counter += 1
        if (inst.ea - shellcode_base == 0x17A0B) and (inst.itype == idaapi.NN_jmpni and inst[0].type == idaapi.o_reg and inst[0] == idautils.procregs.R12):
            current_part = 2
        if (inst.ea - shellcode_base == 0x2F394) and (inst.itype == idaapi.NN_jmpni and inst[0].type == idaapi.o_reg and inst[0] == idautils.procregs.Rbp):
            current_part = 3
        if (inst.ea - shellcode_base == 0x47528) and (inst.itype == idaapi.NN_jmpni and inst[0].type == idaapi.o_reg and inst[0] == idautils.procregs.R13):
            current_part = 4
        if (inst.ea - shellcode_base == 0x5D2DB) and (inst.itype == idaapi.NN_jmpni and inst[0].type == idaapi.o_reg and inst[0] == idautils.procregs.R13):
            current_part = 5
        if (inst.ea - shellcode_base == 0x72319) and (inst.itype == idaapi.NN_jmpni and inst[0].type == idaapi.o_reg and inst[0] == idautils.procregs.R15):
            current_part = 6  # the end
    ...
    inst_log["insts"][inst_counter] = {"rva": inst.ea - shellcode_base, "opcodes": opcode_bytes, "disasm": disasm_line}
    # check
    if unhexlify(opcode_bytes)[0:2] == b'41FF':
        inst_log['insts'][inst_counter]['junk'] = 0
    else:
...
# log base
log_base = "z:\\flareon11\\9_serpentine\\trace"
