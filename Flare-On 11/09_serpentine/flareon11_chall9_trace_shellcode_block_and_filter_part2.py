...
def dump_blocks_new(shellcode_base, exec_handler, RtlLookupFunctionEntryReturn, log_base, next_block_file, patch_target, patch_file_offset):
    ...
    while (hit_jmpni_r12 < 2):  # and current_bock <= 364:
    # for i in range(3):
...
def skip_blocks(shellcode_base, exec_handler, next_block):
    ...
    if next_block > 363:
        ...
        ida_dbg.run_to(shellcode_base + 0x0179FD)
        ida_dbg.wait_for_next_event(ida_dbg.WFNE_SUSP, -1)
    ...
    handler_va, handler_rva = run_to_next_RtlpExecuteHandlerForException(exec_handler, shellcode_base)
    # start skipping
    for i in range(start_block, next_block):
        print(f"[+] Skipping block {i} at VA {handler_va:x} RVA {handler_rva:x}")
#        if i == 363:
#            # special treatment for last block in part 1, we need to cheat around
#            # the test r14, r14 instruction at rva 0x0179FD
#            test_r14_va = shellcode_base + 0x0179FD
#            print(f"[+] Cheating ourselves around test r14 check in block {i} at {test_r14_va:x}")
#            ida_dbg.run_to(shellcode_base + 0x0179FD)
#            ida_dbg.wait_for_next_event(ida_dbg.WFNE_SUSP, -1)
#            idc.set_reg_value(0, "R14")
...
def process_block(shellcode_base, exec_handler, next_block, saved_rsp, handler_va, handler_rva):
    ...
    while (inst.itype != idaapi.NN_hlt) and (hit_jmpni_r12 < 2):
