...
.replace(/\+/g, '-')
.replace(/\//g, '_');
var content = JSON.stringify({'user':user, 'ip':ip});
...
response.writeHead(200, {'Set-Cookie':`session=${sessionid}`,'Content-Type':'application/dson'});
response.end('Hello\x20doge.\n');
} else {
response.writeHead(403, {'Set-Cookie':'session=','Content-Type':'application/dson'});
...
var resp_obj = {'Directory':__dirname,'Listing':[]}
;
try{
...
response.writeHead(200, {'Content-Type': 'application/dson'});
...
var resp_obj = {'Directory': dirname, 'Listing': []};
...
let result =wipe();
var resp_obj = {'wipe':result,'date':new Date().toString()};
response.writeHead(200, {'Content-Type':'application/dson'});
response.end(DSON.stringify(resp_obj));
return;
} 
...
response.writeHead(200, {'Set-Cookie':`session=;\x20path=/;\x20expires=Thu,\x2001\x20Jan\x201970\x2000:00:00\x20GMT`,'Content-Type': 'text/plain'});
...
try{
var options = {hostname: MAIN_DOGE_SERVER, port: 8080, path: `/register?user=${ADMIN}&password=${ADMIN_PASSWORD}`, method: 'GET', timeout: 6000}
;
